...
namespace PiP_Tool.ViewModels
{
    public class PiPModeViewModel : ViewModelBase, ICloseable, IDisposable
    {
        ...
        public ICommand ReselectSelectedWindowCommand { get; }
        ...
        #endregion
        private System.Threading.Timer _checkRedColorTimer;
        ...
        public PiPModeViewModel()
        {
            ...
            ReselectSelectedWindowCommand = new RelayCommand(ReselectSelectedWindowCommandExecute);
        ...
        private void InitSelectedWindow(SelectedWindow selectedWindow)
        {
            ...
            _selectedWindowTitle = selectedWindow.WindowInfo.Title;
        ...
        private void SetPositionFromFile()
        {
            WindowData windowData = WindowDataService.Instance.LoadData(_selectedWindow.WindowInfo.Title);
            if (windowData != null)
            {
                Top = windowData.Top + 30;
        ...
        private void LoadedCommandExecute()
        {
            ...
            // Subscribe to the MouseLeftButtonUp event
            if (thisWindow != null)
            {
                thisWindow.MouseLeftButtonUp += (s, e) =>
                {
                    WindowData windowData = new WindowData();
                    windowData.Title = _selectedWindow.WindowInfo.Title;
                    windowData.Left = (int) thisWindow.Left;
                    windowData.Top = (int) thisWindow.Top;
                    WindowDataService.Instance.SaveData(windowData);
                    Trace.WriteLine($"Window moved to: Top = {thisWindow.Top}, Left = {thisWindow.Left}");
                };
            }
        }
        ...
        /// <summary>
        /// Repip window with the same name that been selected.
        /// </summary>
        private void ReselectSelectedWindowCommandExecute()
        {
            List<WindowInfo> openWindows = ProcessesService.Instance.OpenWindows;
            for (int i = 0; i < openWindows.Count; i++)
            {
                WindowInfo window = openWindows[i];
                if (window.Rect.Width < 500) continue; // skip small game window from EVE-O Preview
                if (window.Title == _selectedWindow.WindowInfo.Title) {
                    _selectedWindow.WindowInfo = window;
                    InitSelectedWindow(_selectedWindow);
                    return;
                }
            }
        }
